...
unique_ptr<Poly> GeneratePolynomialHelper(FILE* data, FILE* log,
	...
	// If generated polynomial satisfies all constraints in sub-domain, return // the polynomial.
	intervals.clear();
...
void FindIndexOfSubDomain(FILE* data, FILE* log, FILE* header,
	...
	double d[3];
	fread(d, sizeof(double), 3, data);
	union {double d; unsigned long x;} binary = {d[0]};
	unsigned long lastNBits = binary.x & (0xFFFFFFFFFFFFFFFF >> (64-N));
	if(lastNBits > index)
	{
	...
	else if(lastNBits < index)
	{
		printf("something catastrophically wrong happened");
		exit(0);
	}
	else
	{
		lbIndex = ftell(data)/(3*sizeof(double));
	}

	while(lastNBits == index & !feof(data))
	{
		fread(d, sizeof(double), 3, data);
		binary.d = d[0];
		lastNBits = binary.x & (0xFFFFFFFFFFFFFFFF >> (64-N));
	}
	fseek(data, -3*sizeof(double), SEEK_CUR);
	ubIndex = ftell(data)/(3*sizeof(double));
}
