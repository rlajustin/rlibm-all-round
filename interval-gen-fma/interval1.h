...
double OutputCompensation1(float, double);
//double OutputCompensation2(float, double, double);
void GuessInitialLbUb1(float x, double roundingLb, double roundingUb, double reducedInput, double& guessLb, double& guessUb);
...
void OneIntervalFile(char** argv)
{
	...
	SortIntervalFile(unsorted, sorted);
}
...
void ComputeReducedInterval1(float input, FILE* file, double corrResult) {
	...
	GuessInitialLbUb1(input, roundingLb, roundingUb, reducedInput, guessLb, guessUb);
	...
	tempResult = OutputCompensation1(input, guessLb);
	//    printf("guess: %.10e\n", tempResult);
	...
	while(step > 0) {
		...
		//	printf("yes\n");
		tempResult = OutputCompensation1(input, dx.d);
	...
	tempResult = OutputCompensation1(input, guessUb);
	// If the initial guess puts us outside of rounding interval, there is
	// nothing more we can do
	if (tempResult < roundingLb || tempResult > roundingUb) {
		CalcRedIntErrorMsg1(input, roundingLb, roundingUb,
