#ifndef _COS_INTERVAL
#define _COS_INTERVAL
...
bool CosGuessInitialLbUbFullRNDZ(double R, double sinK, double cosK, double sinR, double cosR, double totalLB, double totalUB, double& sinLB, double& sinUB, double& cosLB, double& cosUB) {
	Double dx, dx1, dx2;
	if (cosK == 0) {
		cosLB = -1.0e300;
		cosUB = 1.0e300;
	...
	if ((cosK*cosR)+(-sinK*sinR) == 0) {
		return true;
	}
	bool oob = false;
	if ((cosK, cosR)*(-sinK*sinR) < totalLB) {
		oob = true;
	}
	if ((cosK*cosR)+(-sinK*sinR) > totalUB) {
		oob = true;
	}
	if (oob) return false;
	if ((cosK >=0) && (sinK >= 0)) {
		unsigned long step = 0x100000000000;
		while (cosK != 0 && sinK != 0 && sinLB != 0 && step > 0) {
			dx1.d = cosUB;
			dx1.x += step;
			dx2.d = sinLB;
			if (sinLB > 0) {
			...
			if (add(multiply(cosK, dx1.d), multiply(-sinK, dx2.d)) <= totalUB) {
				cosUB = dx1.d;
				sinLB = dx2.d;
			} else if (step > 0) {
				step /= 2;
		...
		while (cosK != 0 && step > 0) {
			dx1.d = cosUB;
			dx1.x += step;
			if (add(multiply(cosK, dx1.d), multiply(-sinK, sinLB)) <= totalUB) {
				cosUB = dx1.d;
			} else if (step > 0) {
				step /= 2;
			}
		...
		while (sinK != 0 && sinLB != 0 && step > 0) {
			...
			if (add(multiply(cosK, cosUB), multiply(-sinK, dx2.d)) <= totalUB) {
				sinLB = dx2.d;
			} else if (step > 0) {
				step /= 2;
			}
		...
		step = 0x100000000000;
		while (cosK != 0 && cosLB != 0 && sinK != 0 && sinUB != 0 && step > 0) {
			dx1.d = cosLB;
			dx1.x -= step;
			dx2.d = sinUB;
			if (sinUB > 0) {
			...
			if (add(multiply(cosK, dx1.d), multiply(-sinK, dx2.d)) >= totalLB) {
				cosLB = dx1.d;
				sinUB = dx2.d;
			} else if (step > 0) {
				step /= 2;
		...
		step = 0x100000000000;
		while (cosK != 0 && cosLB != 0 && step > 0) {
			dx1.d = cosLB;
			dx1.x -= step;
			if (add(multiply(cosK, dx1.d), multiply(-sinK, sinUB)) >= totalLB) {
				cosLB = dx1.d;
			} else if (step > 0) {
				step /= 2;
			}
		...
		while (sinK != 0 && sinUB != 0 && step > 0) {
			...
			if (add(multiply(cosK, cosLB), multiply(-sinK, dx2.d)) >= totalLB) {
				sinUB = dx2.d;
			} else if (step > 0) {
				step /= 2;
			}
	...
	if ((cosK < 0) && (sinK >= 0)) {
		unsigned long step = 0x100000000000;
		while (cosK != 0 && cosLB != 0 && sinK != 0 && sinLB != 0 && step > 0) {
			dx1.d = cosLB;
			dx1.x -= step;
			dx2.d = sinLB;
			if (sinLB > 0) {
		...
		while (sinK != 0 && sinLB != 0 && step > 0) {
			...
			if (add(multiply(cosK, cosLB), multiply(-sinK, dx2.d)) <= totalUB) {
				sinLB = dx2.d;
			} else if (step > 0) {
				step /= 2;
			}
		...
		step = 0x100000000000;
		while (cosK != 0 && sinK != 0 && sinUB != 0 && step > 0) {
			dx1.d = cosUB;
			dx1.x += step;
			dx2.d = sinUB;
			if (sinUB > 0) {
		...
		while (sinK != 0 && sinUB != 0 && step > 0) {
			...
			if (add(multiply(cosK, cosUB), multiply(-sinK, dx2.d)) >= totalLB) {
				sinUB = dx2.d;
			} else if (step > 0) {
				step /= 2;
			}
	...
	if ((cosK >= 0) && (sinK < 0)) {
		...
		while (cosK != 0 && step > 0) {
			dx1.d = cosUB;
			dx1.x += step;
			if (add(multiply(cosK, dx1.d), multiply(-sinK, sinUB)) <= totalUB) {
				cosUB = dx1.d;
			} else if (step > 0) {
				step /= 2;
			}
		...
		while (cosK != 0 && cosLB != 0 && step > 0) {
			dx1.d = cosLB;
			dx1.x -= step;
			if (add(multiply(cosK, dx1.d), multiply(-sinK, sinLB)) >= totalLB) {
				cosLB = dx1.d;
			} else if (step > 0) {
				step /= 2;
			}
...
int main(int argc, char** argv){
	...
	for (count=0x3C490FDB; count<0x80000000; ++count) {
		...
		bool check = CosGuessInitialLbUbFullRNDZ(xp, sinpiK, cospiK, sinpiR, cospiR, totalLB, totalUB, sinpiLB, sinpiUB, cospiLB, cospiUB);
