...
double rlibm34_sinhf(float x)
{
	...
	double sinhL;
	if(R == 0x1.d2fd8bccdp-9) sinhL = 0x1.d2fdcb470f5ccp-9;
	else if(R == 0x1.ff884078ecp-8) sinhL = 0x1.ff8993ae28dadp-8;
	else if(R == 0x1.113e28d466p-7) sinhL = 0x1.113ef7cf95d0dp-7;
	else if(R == 0x1.28e70f8382p-7) sinhL = 0x1.28e818c6b68a5p-7;
	else if(R == 0x1.322aa79dbdp-7) sinhL = 0x1.322bca80046ecp-7;
	else
	{
		sinhL = 0x1.111812e6199c3p-7;
		sinhL = multiply(sinhL, R2);
		sinhL = add(sinhL, 0x1.5555555481c53p-3);
		sinhL = multiply(sinhL, R2);
		sinhL = add(sinhL, 0x1.fffffffffffffp-1);
		sinhL = multiply(sinhL, R);
	}
	double tmp5 = multiply(coshHM, sinhL);

	double coshL;
	if(R == 0x1.d2fd8bccdp-9) coshL = 0x1.000069c999dc6p+0;
	else if(R == 0x1.ff884078ecp-8) coshL = 0x1.0001fe7897ce1p+0;
	else if(R == 0x1.113e28d466p-7) coshL = 0x1.000246c8ff9fp+0;
	else if(R == 0x1.28e70f8382p-7) coshL = 0x1.0002afd7d4c13p+0;
	else if(R == 0x1.322aa79dbdp-7) coshL = 0x1.0002db71c422cp+0;
	else
	{
 		coshL = add(coshL, 0x1.5557df50b52d8p-5);
		coshL = multiply(coshL, R2);
		coshL = add(coshL, 0x1.ffffffff1a5c1p-2);
		coshL = multiply(coshL, R2);
		coshL = add(coshL, 0x1.0000000000001p+0);
	}
