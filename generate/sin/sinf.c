...
double rlibm_sinf(float x) {
	...
	if (b < 0xff000000) {
		...
		if (b < 0x9d000000) {
			...
			if(__builtin_expect(b == 0x86e35bc6, 0)) return 0x1.c5b4ab8p-3*__builtin_copysign(1.0, x);
			if(__builtin_expect(b == 0x9cb0e51c, 0)) return -0x1.851fd78p-14*__builtin_copysign(1.0, x);
			if (b < 0x79921fb6) {
			...
			if(__builtin_expect(b == 0xbe411b04, 0)) return 0x1.58ed0f8p-1*__builtin_copysign(1.0, x);
